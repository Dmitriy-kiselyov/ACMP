...
/**
 * Задача 134
 * За какое мин. время можно добраться из точки А в точку В
 * кол-во вершин N <= 100, кол-во рейсов R <= 10^5
 */
public class Main {
    ...
    public static void main(String[] args) throws IOException {
        ...
        int from = in.nextInt();
        int to = in.nextInt();
        Route[] routes = new Route[in.nextInt()];
        for (int i = 0; i < routes.length; i++) {
            routes[i] = new Route(in.nextInt(), in.nextInt(), in.nextInt(), in.nextInt());
        }

        Graph graph = new Graph(n, from, to, routes);
        int s = graph.solve();
...
class Graph {
    ...
    public Graph(int size, int from, int to, Route[] routes) {
        this.size = size + 1;
        this.from = from;
        this.to = to;
        this.routes = routes;
    ...
    public int solve() {
        ...
        Arrays.sort(routes, Comparator.comparingInt(r -> r.fromTime));
        for (Route route : routes) {
            int time = times[route.from];
            if (time <= route.fromTime) {
                times[route.to] = Math.min(times[route.to], route.toTime);
            }
        }

        return times[to] == Integer.MAX_VALUE ? -1 : times[to];
    }
}

class Route {
    int from, fromTime, to, toTime;

    public Route(int from, int fromTime, int to, int toTime) {
        this.from = from;
